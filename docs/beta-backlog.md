...
| ~~Wave B portal entry surfaces~~ → re-scoped to **tenant-subdomain portal** (ADR-004): host shim + magic-link auth + sync AI triage + guest threads + uploads | S | P0 beta-critical | [Portal PRD](mytradeportal-research/PRD-Customer-Portal.md) · [ADR-004](decisions/ADR-004-tenant-subdomain-portal.md) | done (b047696 UI · 65f696c backend — magic-link auth, sync triage, guest threads, uploads; 93 portal tests pass) |
| ~~Wave C portal home~~ → **portal pages + email sequence**: quotes/invoices/booking pages, quote-ready → booking-confirmed → invoice → payment-received + review-prompt emails with magic links | S | P0 beta-critical | [Portal PRD §4.2](mytradeportal-research/PRD-Customer-Portal.md) · [ADR-004](decisions/ADR-004-tenant-subdomain-portal.md) | done (b047696 pages · 65f696c emails — booking_confirmed + payment_received/review prompt live) |
| S3 wildcard infra: `*.mytradeportal.co.uk` on Railway + Hostinger DNS + IaC vars | S | P0 beta-critical | [ADR-004](decisions/ADR-004-tenant-subdomain-portal.md) | **done (2026-09-14)** — apex consolidated via Hostinger flattened CNAME (`@ → jmclotog.up.railway.app`), freeing the second Railway domain slot; wildcard cert VALID, tenant subdomains serving (e.g. coglabs tenant 200). Production scenario re-run still owed (tracked in GitHub Beta project #4) |
| S3 mobile: review_url onboarding/settings + Share QR wiring | S | P0 beta-critical | [Portal PRD §4.1](mytradeportal-research/PRD-Customer-Portal.md) | done (ae1f3a6 — review link in onboarding + settings; dependency-free QR modal with portal URL + native share; OTA-safe) |
| S4 portal PWA + embeddable widget + QR asset pack | S | P1 plan item | [Portal PRD §4.1–4.2](mytradeportal-research/PRD-Customer-Portal.md) | planned (post-beta-start) |
