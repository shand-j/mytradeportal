...
| S3 wildcard infra: `*.mytradeportal.co.uk` on Railway + Hostinger DNS + IaC vars | S | P0 beta-critical | [ADR-004](decisions/ADR-004-tenant-subdomain-portal.md) | **blocked — Railway Hobby plan allows only 2 custom domains/service (www + apex in use); needs Pro upgrade or apex consolidation** |
