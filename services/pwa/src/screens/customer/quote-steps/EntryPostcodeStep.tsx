...
export function EntryPostcodeStep({
  ...
  const checkArea = () => {
    if (!isValid) return;
    // Mock service-area check: accept the demo postcode and any SK postcode
    const inArea = postcode.toUpperCase().startsWith("SK");
    updateFormData({ postcode: postcode.toUpperCase(), inServiceArea: inArea });
    onPostcodeNext();
  };
