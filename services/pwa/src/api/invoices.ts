...
/** Invoices list for the trade Invoices screen (connected mode). */
export function useInvoicesList() {
  ...
  return {
    invoices: isConnected ? (query.data ?? []).map(mapInvoice) : [],
    isConnected,
    isLoading: config.apiEnabled && query.isLoading,
  };
}

/** A single invoice by id (connected mode), mapped to the app `Invoice` shape. */
