...
# required) and lets the seed script run. The e2e override blanks Supabase so
# auth is local bcrypt/JWT (self-contained, matches CI). Bring up the API and
# its dependencies.
export ENVIRONMENT=development
COMPOSE_FILES="-f docker-compose.yml -f docker-compose.e2e.yml"
echo "Starting API stack (dev)..."
...
E2E_TENANT_SLUG="${TENANT_SLUG}" \
  node "${REPO_ROOT}/services/pwa/e2e/seed-e2e.mjs"
