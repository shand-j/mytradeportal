...
export function JobCategoryStep({ formData, updateFormData, onNext, business }: StepPropsWithBusiness) {
  ...
  const enabledKeys = new Set(business?.businessServices?.length ? business.businessServices : ALL_CATEGORIES.map((c) => c.key));
  // "Something else" is always available and opens a free-text description.
  enabledKeys.add("other");
