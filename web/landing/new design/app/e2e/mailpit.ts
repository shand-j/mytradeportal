...
//
// Ported from mobile/e2e/mailpit.ts — keep the two in sync.
...
/** Assert that NO email to `to` (optionally matching a subject substring)
 * arrives within `windowMs` — the negative/no-leak email assertion. */
export async function assertNoEmail(
  to: string,
  opts: { subjectIncludes?: string; windowMs?: number } = {},
): Promise<void> {
  if (!mailpitConfigured()) {
    throw new Error("MAILPIT_URL / MAILPIT_BASIC_AUTH not set");
  }
  ...
  for (;;) {
    ...
    if (matches.length > 0) {
      throw new Error(
        `expected no email to ${to}` +
          (opts.subjectIncludes ? ` with subject containing "${opts.subjectIncludes}"` : "") +
          ` but found ${matches.length} (e.g. "${matches[0].Subject}")`,
      );
    }
